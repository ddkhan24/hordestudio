...
function config(raw={}) {raw=raw&&typeof raw==='object'?raw:{};return {
 ...
 frame:{openerMode:raw.frame?.openerMode==='vh_first'?'vh_first':'player_first',openingMessage:str(raw.frame?.openingMessage,4000),openingDelayMinutes:clamp(raw.frame?.openingDelayMinutes??1,0,1440),openerScenario:str(raw.frame?.openerScenario,2000),mode:['direct','dating','private_social','public_social'].includes(raw.frame?.mode)?raw.frame.mode:'direct',acceptRequests:raw.frame?.acceptRequests!==false,minComfort:clamp(raw.frame?.minComfort??-20,-100,100),requestMinutes:clamp(raw.frame?.requestMinutes??2,0,1440)},
