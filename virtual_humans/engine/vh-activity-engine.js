...
(function (root, factory) {
    ...
    function normalize(raw = {}) {
        return {
            ...
            goals: (Array.isArray(raw?.goals) ? raw.goals : []).filter(goal => goal && typeof goal === 'object').slice(-60).map(goal => ({
                ...
                survivalKind:['sleep','food'].includes(goal.survivalKind)?goal.survivalKind:'',unpaidCost:clamp(goal.unpaidCost,0,1000000),
    ...
    function addGoal(state, kind, id, now, label = '') {
        ...
        const goal = { id, kind, meetingId: '', startedAt:0, requiredPlaceId:'', projectId: '', learningEnabled: false, learningRecorded: false, definitionKey: '', opportunityId: '', participantId: '', survivalKind:'', unpaidCost:0, commitmentId: '', notBefore: 0, expiresAt: 0, minEnergy: 0, label: String(recipe.label).slice(0, 200), priority: recipe.priority,
