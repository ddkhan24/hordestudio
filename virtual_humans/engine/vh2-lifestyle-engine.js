...
function ensure(c){
 c.vh2Finance ||= {policy:{enabled:false,incomePerHour:0,dailyIncome:0,dailyExpense:0,debtRepaymentRate:25,workPlaceIds:[]},lastAt:null,nextDue:null,trackedBalance:null,incomeRemainder:0,repaymentRemainder:0,unpaid:0,ledger:[],sequence:0};
 c.vh2Finance.policy.debtRepaymentRate??=25;c.vh2Finance.repaymentRemainder??=0;
...
function repay(c,at,inflow){const r=c.vh2Finance,w=c.lifeRuntime.world;if(r.unpaid<=0||inflow<=0)return 0;const accrued=r.repaymentRemainder+inflow*100*Math.max(0,Math.min(100,Number(r.policy.debtRepaymentRate??25)))/100,whole=Math.floor(accrued+1e-8),cents=Math.min(whole,Math.round(r.unpaid*100),Math.round(w.balance*100));r.repaymentRemainder=accrued-whole;if(cents<=0)return 0;const amount=cents/100;w.balance=Math.round((w.balance-amount)*100)/100;r.unpaid=Math.round((r.unpaid-amount)*100)/100;entry(c,at,'debt_repayment',-amount,`Applied ${r.policy.debtRepaymentRate}% of new income to outstanding expenses; remaining ${r.unpaid.toFixed(2)}.`);return amount;}
function finance(c,at,baseline){ensure(c);const r=c.vh2Finance,w=c.lifeRuntime.world,p=r.policy;if(!p.enabled)return;
 ...
 if(!w.journey&&p.workPlaceIds.includes(w.placeId)&&baseline.placeId===w.placeId&&baseline.source==='schedule'&&baseline.availability!=='asleep'){
  ...
  if(whole){w.balance=Math.round((w.balance+whole/100)*100)/100;entry(c,at,'earned_income',whole/100,'Income for actual time at a configured work place.');repay(c,at,whole/100);}
 }
 if(at>=r.nextDue){const periods=Math.floor((at-r.nextDue)/86400000)+1;r.nextDue+=periods*86400000;
  const income=periods*p.dailyIncome;if(income){w.balance+=income;entry(c,at,'recurring_income',income,'Configured recurring income.');repay(c,at,income);}
...
module.exports={ensure,entry,repay,finance,prepareCloset,repetition,wore,garmentName,resolveGarmentColor};
