...
function advanceCompanionActivities(companion, nowMs = Date.now(), options = {}) {
    ...
    while (cursor + 60000 <= nowMs) {
        ...
        const delta = VHActivityEngine.advance(activities, cursor + 60000, {
            ...
            availability: situation.source==='sleep_transition'?'private':situation.availability, label: situation.label, energy: Math.max(0,dynamics.energy-(dynamics.sleep?.stage==='drowsy'?15:0)), stress:dynamics.stress, hunger: dynamics.hunger, sleepPressure:dynamics.sleep?.pressure||0, socialNeed: dynamics.socialNeed, availablePeople,
