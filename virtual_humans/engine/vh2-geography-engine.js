...
function propose(c,at,localAt){
 ...
 for(const goal of a.goals){
  ...
  if(goal.status==='completed'&&!g.processed.includes(goal.id)){
   ...
   if(goal.unpaidCost>0){const finance=require('./vh2-lifestyle-engine');finance.ensure(c);c.vh2Finance.unpaid=Math.round((c.vh2Finance.unpaid+goal.unpaidCost)*100)/100;finance.entry(c,goal.completedAt||at,'essential_food_credit',0,`Food cost ${goal.unpaidCost.toFixed(2)} was recorded as unpaid rather than suppressing an essential meal.`);}
...
g.lastHungerBand=Math.floor(d.hunger/10);
 ...
 const homeId=c.vh2Travel?.residenceId||c.lifeProfile.places.find(place=>place.kind==='home')?.id;
 if(homeId&&(d.sleep?.pressure||0)>=75&&!a.goals.some(goal=>goal.survivalKind==='sleep'&&!['completed','abandoned'].includes(goal.status))){
  const goal=activity.addGoal(a,'recovery',`geo:sleep:${homeId}:${Math.floor(at/(6*3600000))}`,at,'Return home and make room for sleep');
  if(goal){Object.assign(goal,{definitionKey:'sleep_recovery',survivalKind:'sleep',requiredPlaceId:homeId,expiresAt:at+12*3600000,priority:100,reason:'Sleep pressure is high; staying at a resting place must pre-empt optional activity.'});goal.steps=[{label:'Wind down at home',durationMs:60*MIN,progressMs:0,energy:10,stress:-8,hunger:0,costs:{},produces:{},charged:false}];}
 }
 ...
 for(const place of nearby(c,32)){
  ...
  const homeMeal=kind==='meal'&&place.kind==='home',missingIngredients=homeMeal&&!(a.resources.ingredients>=1);
  const amount=kind==='meal'&&!homeMeal?(meta.mealCost??g.unknownMealCost??10):0;
  const emergency=kind==='meal'&&d.hunger>=90&&(missingIngredients||amount>(w.balance||0));
  if(missingIngredients&&!emergency){records.push({placeId:place.id,excluded:'no ingredients at home'});continue;}
  if(amount>(w.balance||0)&&!emergency){records.push({placeId:place.id,excluded:'unaffordable'});continue;}
  ...
  goal.label=(exercise?'Work out at ':kind==='meal'?'Eat at ':kind==='recovery'?'Rest at ':'Spend time at ')+place.label;goal.requiredPlaceId=place.id;goal.expiresAt=at+2*3600000;goal.survivalKind=kind==='meal'?'food':'';
  goal.unpaidCost=emergency?(homeMeal?Number(meta.mealCost??g.unknownMealCost??10):amount):0;goal.essentialCredit=emergency;
  goal.reason=emergency?'Critical hunger makes this essential; the unavailable food cost is recorded as unpaid rather than pretending the meal is free.':`${capability} is available here; ${hours===null?'opening hours unknown':'open according to saved hours'}${kind==='meal'&&meta.mealCost==null?'; meal cost is a simulation estimate':''}.`;
  goal.steps=[{label:goal.label,durationMs:(kind==='meal'?20:25)*MIN,progressMs:0,energy:kind==='meal'?6:kind==='recovery'?15:exercise?-12:-2,stress:-8,hunger:kind==='meal'?-60:0,costs:emergency?{}:homeMeal?{ingredients:1}:amount?{vh_cash:Math.round(amount*100)}:{},produces:{},charged:false}];
...
function routeGoal(c,goal,at,localAt){
 ...
 const essentialTravel=goal.survivalKind==='sleep'||goal.essentialCredit===true;
 let legs=transport.route(c,actor,goal.requiredPlaceId,at),creditCost=0;
 if(legs===null&&essentialTravel){const essentialActor={...actor,balance:Number.MAX_SAFE_INTEGER,policy:{...actor.policy}};legs=transport.route(c,essentialActor,goal.requiredPlaceId,at);if(legs)creditCost=Math.max(0,legs.reduce((sum,leg)=>sum+Number(leg.cost||0),0)-Math.max(0,actor.balance));}
 ...
 const homeId=c.vh2Travel?.residenceId||c.lifeProfile.places.find(place=>place.kind==='home')?.id;
 if(!essentialTravel&&homeId&&goal.requiredPlaceId!==homeId){const returnActor={...actor,placeId:goal.requiredPlaceId,balance:actor.balance-legs.reduce((sum,leg)=>sum+Number(leg.cost||0),0),policy:{...actor.policy}};for(const leg of legs){if(leg.mode==='DRIVE')returnActor.carPlaceId=leg.to;if(leg.mode==='BICYCLE')returnActor.bikePlaceId=leg.to;}if(transport.route(c,returnActor,homeId,at+minutes*MIN)===null)return null;}
 return {geographic:true,legs,creditCost,penalty:minutes*.35+money*.5};
}
function start(c,goal,at,route){
 ...
 if(route.creditCost>0){const finance=require('./vh2-lifestyle-engine');finance.ensure(c);c.lifeRuntime.world.balance=Math.round((c.lifeRuntime.world.balance+route.creditCost)*100)/100;c.vh2Finance.unpaid=Math.round((c.vh2Finance.unpaid+route.creditCost)*100)/100;finance.entry(c,at,'essential_transport_credit',0,`Essential travel cost ${route.creditCost.toFixed(2)} was recorded as unpaid so food or sleep recovery was not blocked.`);}
