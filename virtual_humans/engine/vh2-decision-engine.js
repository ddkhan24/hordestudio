...
function select(runtime,candidates,now,context,score){
 ...
 if(context.sleepPressure>=85&&feasible.some(g=>g.survivalKind==='sleep')){feasible=feasible.filter(g=>g.survivalKind==='sleep');urgency=urgency?urgency+'+urgent_sleep':'urgent_sleep';}
 ...
 const temperature=Math.max(0,Math.min(50,p.temperature*Math.max(.5,Math.min(1.5,Number(context.temperatureMultiplier)||1))));
 const weights=temperature===0?scored.map(g=>g.score===max?1:0):scored.map(g=>Math.exp((g.score-max)/temperature));
 const total=weights.reduce((a,b)=>a+b,0);
 const probabilities=weights.map(w=>temperature===0?w/total:(1-p.exploration)*w/total+p.exploration/weights.length);
