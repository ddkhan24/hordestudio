...
function advance(c,now){
 ...
 const pair=r.pairs[p.id];
  ...
  // Completed contact activities are direct shared-time evidence even when no
  // calendar plan object was involved. Co-location by itself still earns nothing.
  for(const goal of c.lifeRuntime?.activities?.goals||[]){
   const token='activity:'+goal.id;if(goal.kind!=='contact'||goal.participantId!==p.id||goal.status!=='completed'||pair.processed.includes(token))continue;
   pair.processed.push(token);
   const at=goal.completedAt??now,duration=clamp((goal.steps||[]).reduce((n,step)=>n+Number(step.durationMs||0),0)/60000,1,120);
   ...
   for(const [which,s,load] of [['self',pair.self,stress],['other',pair.other,otherStress]]){
    const d=disposition(c,p,which),comfort=1-load/70,delta=clamp(comfort*(.3+d.openness/100)*Math.min(1,duration/30),-1,1),previous=s.stage;
    s.strain=clamp((s.strain||0)+(delta<0?-delta:-delta*.5),0,100);if(delta<0)s.trust=clamp(s.trust+delta*.15,-100,100);
    s.warmth=clamp(s.warmth+delta,-100,100);s.familiarity=clamp(s.familiarity+Math.min(1,duration/30),0,100);s.meetings++;s.firstMeetingAt??=at;s.lastMeetingAt=at;
    if(s.stage==='authored')s.stage='acquaintance';if(s.stage==='distant'&&comfort>0)s.stage='acquaintance';if((s.strain||0)>=pol.strainThreshold)s.stage='estranged';
    r.events.push({id:`bond:${c.id}:${++r.sequence}`,kind:previous===s.stage?'social_bond_experience':'social_bond',at,personId:p.id,direction:which,stage:s.stage,summary:which==='self'?`Completed contact with ${p.name} changed the relationship’s recorded experience.`:'The supporting person recorded the completed contact independently.'});
   }
  }
  ...
  const retained=new Set([...(c.vh2Plans?.plans||[]).map(p=>p.id),...(c.lifeRuntime?.activities?.goals||[]).map(goal=>'activity:'+goal.id)]);pair.processed=pair.processed.filter(id=>retained.has(id));
 }
