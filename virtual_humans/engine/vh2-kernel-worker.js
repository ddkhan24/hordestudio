...
const embodiment=require('./vh-embodiment-engine');
const cognition=require('./vh-cognition-engine');
const mind=require('./vh-mind-engine');
...
function create(name, id, now, deferAdvance=false) {
    ...
    return deferAdvance?c:advance(c, now);
}
function advance(c, now) {
    ...
    core.advanceCompanionActivities(c,now,{selectAction:(candidates,at,context,score)=>{const blocked=candidates.filter(goal=>!embodiment.activityFeasible(c.embodimentProfile,goal)).map(goal=>({id:goal.id,reason:'body_access_infeasible'})),feasible=candidates.filter(goal=>embodiment.activityFeasible(c.embodimentProfile,goal));return decisions.select(c.vh2Decision,feasible,at,{...context,excluded:[...(context.excluded||[]),...blocked],temperatureMultiplier:cognition.mechanics(c.cognitionProfile).decisionTemperatureMultiplier},goal=>{const base=score(goal),experience=psychology.experienceScore(c,goal),intention=VH2Geography.preference(c,goal),reflection=psychology.reflectionScore(c.vh2Psychology,goal.requiredPlaceId),healthPreference=health.preference(c,goal),storyPreference=story.preference(c,goal,at),bodyPreference=embodiment.activityScore(c.embodimentProfile,goal),cognitionPreference=cognition.activityScore(c.cognitionProfile,goal),mindPreference=mind.activityScore(c.mindProfile,goal,{runtime:c.mindRuntime,now:at,age:core.companionCurrentAge(c),libidoEnabled:core.companionSexualSystemActive(c)});return {...base,score:base.score+experience+intention+reflection+healthPreference+storyPreference+bodyPreference+cognitionPreference+mindPreference,components:{...base.components,experience,intention,reflection,health:healthPreference,story:storyPreference,body:bodyPreference,cognition:cognitionPreference,mind:mindPreference}};});}});
    ...
    if(c.mindProfile){c.mindProfile=mind.normalizeProfile(c.mindProfile);c.mindRuntime=mind.normalizeRuntime(c.mindRuntime,now);c.mindContext=mind.contextSnapshot(c.mindProfile,c.mindRuntime,{now,age:core.companionCurrentAge(c),libidoEnabled:core.companionSexualSystemActive(c)});}
    if(c.embodimentProfile){c.embodimentProfile=embodiment.normalizeProfile(c.embodimentProfile);c.embodimentContext=embodiment.contextSnapshot(c.embodimentProfile,c.lifeProfile);}
    if(c.cognitionProfile){c.cognitionProfile=cognition.normalizeProfile(c.cognitionProfile);c.cognitionContext=cognition.contextSnapshot(c.cognitionProfile);}
    return c;
}
function run(input) {
    ...
    try {
        ...
        if(input.create&&input.profile){
            ...
            if(input.deferAdvance!==true)c=advance(c,input.now);
        }
        ...
        if(inbox&&c.mindProfile){
            const perceived=[...(inbox.messages||[])].reverse().find(m=>m.role==='user'&&m.readAt>0&&m.readAt<=input.now);
            if(perceived){const result=mind.evaluate(c.mindProfile,c.mindRuntime,perceived.text,{now:input.now,inputId:perceived.id,age:core.companionCurrentAge(c),libidoEnabled:core.companionSexualSystemActive(c)});c.mindProfile=result.profile;c.mindRuntime=result.runtime;
                c.humanDynamics.anger=core.livingClamp(c.humanDynamics.anger+result.deltas.anger,0,100);c.humanDynamics.stress=core.livingClamp(c.humanDynamics.stress+result.deltas.stress,0,100);c.humanDynamics.inhibition=core.livingClamp(c.humanDynamics.inhibition+result.deltas.inhibition,0,100);
                if(result.deltas.sexualArousal&&core.companionSexualSystemActive(c)){c.humanDynamics.sexualArousal=core.livingClamp(c.humanDynamics.sexualArousal+result.deltas.sexualArousal,0,100);c.humanDynamics.desire=core.livingClamp(c.humanDynamics.desire+result.deltas.sexualArousal*.2,0,100);}
                c.mindContext=mind.contextSnapshot(c.mindProfile,c.mindRuntime,{now:input.now,age:core.companionCurrentAge(c),libidoEnabled:core.companionSexualSystemActive(c)});}
        }
