...
function startAction(c,a,choice,at,localAt){
 ...
 if(choice.kind==='eat'){
  const mealCost=Number(choice.mealCost||0);
  if(a.balance<mealCost&&a.hunger<90){a.blockedReason='Cannot afford this meal.';return;}
  const paid=Math.min(a.balance,mealCost),unpaid=mealCost-paid;
  a.balance=Math.round((a.balance-paid)*100)/100;
  if(unpaid){a.unpaidExpenses=Math.round(((a.unpaidExpenses||0)+unpaid)*100)/100;emit(c,a,'person_essential_meal_unpaid',at,{due:mealCost,paid,outstanding:a.unpaidExpenses});}
 }
 ...
 a.blockedReason='';emit(c,a,'person_action_started',at,{action:choice.kind,placeId:a.placeId,planId:choice.planId||null,cost:choice.kind==='eat'?Number(choice.mealCost||0):choice.entryCost||0});
}
function choose(c,a,at,localAt){
 ...
 const add=(kind,placeId,minutes,score,extra={})=>{
  ...
  const mealCost=kind==='eat'&&placeId!==p.homePlaceId?Number(p.mealCost||0):0;
  if(kind==='eat'&&cost+mealCost>a.balance&&a.hunger<90||cost+(extra.entryCost||0)>a.balance)return;
  if(extra.deadline&&at+(travel+minutes)*MIN>extra.deadline)return;
  candidates.push({kind,placeId,minutes,path,mealCost,score:score-travel*.7-cost*.15+(kind==='eat'&&placeId===p.homePlaceId?12:0),...extra});
 };
