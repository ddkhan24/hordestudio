...
def command(service,db,world,revision,state,body):
 ...
 if body['type']=='configure_personal_preferences':
  ...
  age=_vh_import_module('.vh2_calendar',__package__).current_age(state['truth']['companion'])
 ...
 if body['type']=='configure_expression_profile':
  ...
  opening=str(c.get('openingMessage','')).strip()
  if c.get('openingMode')=='vh_first' and opening and not after['communication']['messages'] and not c.get('continuityRuntime',{}).get('originScenarioConsumedAt'):
   c.setdefault('continuityRuntime',{})['originScenarioConsumedAt']=after['simAt']
   opener_id=str(uuid.uuid5(uuid.NAMESPACE_URL,'vh2-authored-opening:'+world+':'+after['communication']['personaId']))
   service.apply_reply(after,opener_id,opening,[],'authored_opening',revision+1)
