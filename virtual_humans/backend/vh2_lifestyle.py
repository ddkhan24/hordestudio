...
COMMANDS=('configure_health_policy','configure_live_routing','remove_life_entry','apply_life_proposal','configure_institution','remove_institution','upsert_place','remove_place','record_route','configure_place_context','set_residence','set_hometown','configure_finances','settle_expenses','configure_closet','add_closet_style','remove_closet_style')

def place_removal_blockers(c,ident):
 """Inspect current configuration/work, never historical visits or event records."""
 blockers=[]
 def scan(value,path):
  if isinstance(value,dict):
   for key,item in value.items():
    reference=key in ('placeId','placeIds','requiredPlaceId','residenceId','hometownId','from','to') or key.endswith(('PlaceId','PlaceIds'))
    if reference and (item==ident or isinstance(item,list) and ident in item):blockers.append(path+'.'+key)
    elif isinstance(item,(dict,list)):scan(item,path+'.'+key)
  elif isinstance(value,list):
   for i,item in enumerate(value):scan(item,path+'['+str(i)+']')
 life=c['lifeProfile'];world=c['lifeRuntime']['world'];travel=c.get('vh2Travel',{})
 scan({k:v for k,v in life.items() if k not in ('places','travelLegs','referencePlan','rooms')},'Life setup')
 scan({k:world.get(k) for k in ('placeId','journey','carPlaceId','bikePlaceId','people')},'Current location or journey')
 scan({k:travel.get(k) for k in ('residenceId','hometownId','carPlaceId','bikePlaceId')},'Residence or vehicle')
 scan([t for t in travel.get('trips',[]) if t.get('status') not in ('completed','cancelled','expired')],'Trips')
 scan([g for g in c['lifeRuntime'].get('activities',{}).get('goals',[]) if g.get('status') not in ('completed','abandoned','cancelled','expired')],'Activities')
 for person,actor in c.get('vh2People',{}).get('actors',{}).items():
  scan({k:actor.get(k) for k in ('policy','placeId','carPlaceId','bikePlaceId','journey','pending','remaining','action')},'Person '+person)
 scan(c.get('vh2People',{}).get('setup',{}),'People setup')
 scan(c.get('vh2NpcTravel',{}).get('people',{}),'Supporting people location')
 scan(c.get('vh2Population',{}).get('residents',[]),'Local people')
 scan([s for s in c.get('vh2Transport',{}).get('services',[]) if s.get('status')!='cancelled'],'Scheduled transport')
 scan(c.get('vh2Finance',{}).get('policy',{}),'Money settings')
 scan([p for p in c.get('vh2Plans',{}).get('plans',[]) if p.get('status') not in ('completed','cancelled','declined','expired')],'Shared plans')
 scan(c.get('vh2Signals',{}).get('sources',[]),'World feeds')
 return list(dict.fromkeys(blockers))

def remove_place(c,ident,present=None):
 from .vh2_runtime import Conflict
 if not isinstance(ident,str) or not any(p['id']==ident for p in c['lifeProfile']['places']):raise ValueError('Choose an existing place.')
 blockers=place_removal_blockers(c,ident)
 # A non-spatial life still has an authoritative current situation. Its world
 # location can be empty when transport is disabled; do not delete the place
 # shown in that situation just because no journey controller owns it.
 if (present or {}).get('placeId')==ident:blockers.insert(0,'current activity location')
 if blockers:raise Conflict('This place is still used by '+', '.join(blockers[:6])+'. Move to another place or edit these linked settings first. Pausing alone does not remove those links.')
 life=c['lifeProfile'];life['places']=[p for p in life['places'] if p['id']!=ident]
 life['travelLegs']=[r for r in life.get('travelLegs',[]) if ident not in (r.get('from'),r.get('to'))]
 visual=c.get('vh2Visual',{});zones={z['id'] for z in visual.get('zones',[]) if z['placeId']==ident}
 visual['zones']=[z for z in visual.get('zones',[]) if z['id'] not in zones]
 if visual.get('zoneId') in zones:visual['zoneId']=None
 for entry in c.get('vh2Assets',{}).get('entries',[]):
  if entry.get('role')=='place' and entry.get('entityId')==ident or entry.get('role')=='zone' and entry.get('entityId') in zones:entry['status']='archived'
 for section in ('vh2Travel','vh2Geography'):c.get(section,{}).get('places',{}).pop(ident,None)
 geography=c.get('vh2Geography',{})
 if 'knownPlaceIds' in geography:geography['knownPlaceIds']=[p for p in geography['knownPlaceIds'] if p!=ident]
 # Full old entities, reference revisions and historical visits remain in the
 # event log and frozen captures. Only authoring/current lookup rows are removed.
 if 'rooms' in life:life['rooms']=[r for r in life['rooms'] if r.get('placeId')!=ident]
 if 'referencePlan' in life:life['referencePlan']=[r for r in life['referencePlan'] if r.get('entityId')!=ident and r.get('entityId') not in zones]
 if 'vh2ReferencePlan' in c:c['vh2ReferencePlan']=[r for r in c['vh2ReferencePlan'] if r.get('entityId')!=ident and r.get('entityId') not in zones]
...
def command(service,db,world,revision,state,body):
 ...
 if kind=='configure_health_policy':
  ...
  if body.get('baseSetupVersion')!=c.get('vh2SetupVersion',0):raise Conflict('Life setup changed. Reopen this place before removing it.')
  remove_place(c,body.get('placeId'),after['truth'].get('present'))
  ...
  p=body.get('policy')
  required={'enabled','incomePerHour','dailyIncome','dailyExpense','workPlaceIds'};allowed=required|{'debtRepaymentRate'}
  if not isinstance(p,dict) or not required<=set(p) or set(p)-allowed or type(p['enabled']) is not bool:raise ValueError('Provide the complete finance policy.')
  ...
  p['debtRepaymentRate']=p.get('debtRepaymentRate',25)
  if type(p['debtRepaymentRate']) not in (int,float) or not 0<=p['debtRepaymentRate']<=100:raise ValueError('Debt repayment rate must be 0–100 percent.')
...
def apply_proposal(service,db,world,revision,state,body):
 ...
 if 'personalCalendar' in proposal:
  merged['personalCalendar']=_vh_import_module('.vh2_calendar',__package__).validate_personal(proposal['personalCalendar'],{**c,'lifeProfile':merged},_vh_import_module('.vh2_calendar',__package__).local_datetime(state).date())
 for row in merged.get('weeklySchedule',[]):
  _vh_import_module('.vh2_calendar',__package__).validate_dates(row)
...
def apply_complete_policies(service,db,world,revision,state,proposal):
 ...
 from . import vh2_player; from . import vh2_geography; from . import vh2_population; from . import vh2_people; from . import vh2_plans
