...
def command(service,db,world,revision,state,body):
 ...
 if kind=='configure_geographic_life':
  ...
  pack=_vh_import_module('.vh2_world_packs',__package__).get(db,body['packId']) if body.get('packId') else body.get('pack')
  ...
  if replacement is not None:
   ...
   previous_pack=_vh_import_module('.vh2_world_packs',__package__).get(db,replacement)
