...
COMMANDS=('save_place_zone','remove_place_zone','enter_place_zone')
...
def command(service,db,world,revision,state,body):
 ...
 elif body['type']=='remove_place_zone':
  ident=body.get('zoneId');zone=next((z for z in v['zones'] if z['id']==ident),None)
  if not zone:raise ValueError('Choose an existing room.')
  if body.get('zoneRevision')!=zone['revision']:raise Conflict('This room changed. Reopen it before removing it.')
  v['zones']=[z for z in v['zones'] if z['id']!=ident]
  if v.get('zoneId')==ident:v['zoneId']=None
  for entry in c.get('vh2Assets',{}).get('entries',[]):
   if entry.get('role')=='zone' and entry.get('entityId')==ident:entry['status']='archived'
  life=c['lifeProfile']
  if 'rooms' in life:life['rooms']=[r for r in life['rooms'] if r.get('id')!=ident]
  if 'referencePlan' in life:life['referencePlan']=[r for r in life['referencePlan'] if r.get('entityId')!=ident]
  if 'vh2ReferencePlan' in c:c['vh2ReferencePlan']=[r for r in c['vh2ReferencePlan'] if r.get('entityId')!=ident]
