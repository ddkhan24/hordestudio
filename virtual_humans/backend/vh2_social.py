...
def command(service,db,world_id,revision,state,body):
    ...
    if kind in ('import_starter_post','import_starter_gallery'):
        ...
        if kind=='withdraw_post':
            ...
            primary=_vh_import_module('.vh2_conversations',__package__).canonical(after)['communication']['personaId']
...
def persona_post(post,state,persona_id=None):
    primary=_vh_import_module('.vh2_conversations',__package__).canonical(state)['communication']['personaId']
