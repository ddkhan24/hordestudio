...
class ProviderStore:
    ...
    def usage(self,db,scope=None):
        """Attribute old and new charges through immutable submitted job versions.

        Do not use a world's current scope: imports/configuration changes cannot
        move a past charge to another character. Unknown old charges remain in
        the legacy unscoped allowance rather than charging every character.
        """
        day=self.service.clock()//DAY*DAY
        rows=db.execute("""SELECT
            CASE WHEN d.id IS NOT NULL THEN 'dialogue'
                 WHEN social.id IS NOT NULL OR story.id IS NOT NULL THEN 'background'
                 ELSE 'unattributed' END AS category,
            COUNT(*) AS used
            FROM dialogue_usage AS u
            LEFT JOIN dialogue_jobs AS d ON d.id=u.job_id
            LEFT JOIN vh2_social_jobs AS social ON 'social:'||social.id=u.job_id
            LEFT JOIN vh2_story_jobs AS story ON story.id=u.job_id
            LEFT JOIN dialogue_providers AS p ON p.id=COALESCE(
                json_extract(d.snapshot,'$.provider.version'),
                json_extract(social.snapshot,'$.providerId'),
                json_extract(story.snapshot,'$.providerId'))
            WHERE u.at>=? AND u.at<? AND COALESCE(
                json_extract(d.snapshot,'$.provider.scope'),json_extract(p.config,'$.scope')) IS ?
            GROUP BY category""",(day,day+DAY,scope)).fetchall()
        counts={'dialogue':0,'background':0,'unattributed':0}
