...
def parse(raw,source,now):
 ...
 if decoded.lstrip().startswith('BEGIN:VCALENDAR'):
  return _vh_import_module('.vh2_calendar',__package__).parse(decoded.strip(),source,now)
 source['warning']=''
...
def ingest(c,source,raw,now,wall):
 if source.get('kind')=='gtfs_rt':return _vh_import_module('.vh2_realtime',__package__).ingest(c,source,raw,now,wall)
 if source.get('kind') in ('gtfs','flights_json','aviationstack'):
  incoming=_vh_import_module('.vh2_flights',__package__).parse(raw,source,now) if source.get('kind')=='aviationstack' else _vh_import_module('.vh2_live_data',__package__).flights(raw,source,now) if source.get('kind')=='flights_json' else _vh_import_module('.vh2_gtfs',__package__).parse(raw,source,now);transport=c.setdefault('vh2Transport',{'services':[],'sequence':0});active=c['lifeRuntime']['world'].get('journey') or {}
 ...
 r=ensure(c);incoming=_vh_import_module('.vh2_ticketmaster',__package__).parse(raw,source,now) if source.get('kind')=='ticketmaster' else _vh_import_module('.vh2_live_data',__package__).sports(raw,source,now) if source.get('kind')=='openliga' else parse(raw,source,now);old={s['id']:s for s in r['signals'] if s['expiresAt']>now}
...
def command(service,db,world_id,revision,state,body):
 ...
 if kind=='configure_world_feed':
  url=_vh_import_module('.vh2_ticketmaster',__package__).URL if body.get('kind')=='ticketmaster' else _vh_import_module('.vh2_flights',__package__).URL if body.get('kind')=='aviationstack' else body.get('url');url_parts(url);place=body.get('placeId') or None;hours=body.get('freshHours',48);interval=body.get('intervalMinutes',60);tags=body.get('tags',[])
  ...
  event_filters=_vh_import_module('.vh2_ticketmaster',__package__).filters(body,{p['id'] for p in c['lifeProfile']['places']}) if body.get('kind')=='ticketmaster' else None
  ...
  with service.connect() as db:
   ...
   try:
    ...
    if source.get('kind') in ('aviationstack','ticketmaster'):
     adapter=_vh_import_module('.vh2_ticketmaster' if source['kind']=='ticketmaster' else '.vh2_flights',__package__)
     raw,version=raw
