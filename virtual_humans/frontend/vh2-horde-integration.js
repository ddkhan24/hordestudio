...
async function vh2OpenReplyDetails(companion,timeline=getActiveCompanionTimeline(companion.id)){
 ...
 try{const result=await mcpBridgeRequest('/vh2/dialogue-jobs?worldId='+encodeURIComponent(timeline.vh2.worldId));if(!d.open)return;
  for(const job of result.jobs||[]){const row=document.createElement('details'),title=document.createElement('summary');title.textContent=(job.model||'Offline fixture')+' · '+job.status+' · '+new Date(job.created_at).toLocaleString();row.append(title);
   ...
   const prompt=document.createElement('details'),heading=document.createElement('summary');heading.textContent='Frozen prompt text (redacted preview)';prompt.append(heading);const note=document.createElement('p');note.textContent='Provider keys, headers and media attachments are omitted.'+(job.promptTruncated?' Preview shortened to 24,000 characters.':'');prompt.append(note);const pre=document.createElement('pre');pre.style.whiteSpace='pre-wrap';pre.style.overflowWrap='anywhere';pre.textContent=(job.promptPreview||[]).map(m=>m.role.toUpperCase()+':\n'+m.content).join('\n\n');prompt.append(pre);row.append(prompt);f.append(row);
  }status.textContent=result.jobs?.length?'Open an attempt to inspect its usage, errors and request text.':'No dialogue attempts recorded for this life.';
...
async function vh2PollOwned(companion,timeline,options={}){
    ...
    try{
        ...
        const optional=await Promise.allSettled([mcpBridgeRequest('/vh2/provider-jobs?worldId='+encodeURIComponent(link.worldId)),mcpBridgeRequest('/vh2/image-provider?scope='+encodeURIComponent('horde:'+companion.id)),mcpBridgeRequest('/vh2/flight-provider?scope='+encodeURIComponent('horde:'+companion.id)),mcpBridgeRequest('/vh2/dialogue-provider?scope='+encodeURIComponent('horde:'+companion.id))]);
        ...
        link.optionalProviderErrors=optional.map((result,index)=>result.status==='rejected'?{capability:['Background jobs','Background images','Flights','Text request budgets'][index],message:String(result.reason?.message||'Unavailable')}:null).filter(Boolean);
        const workerSignature=JSON.stringify([workerStatus,imageProvider,flightProvider,textProvider]);
        ...
        link.workerSignature=workerSignature;link.providerJobs=workerStatus.jobs;link.imageProvider=imageProvider;link.flightProvider=flightProvider;link.textProvider=textProvider;link.imageStudioFingerprint??=vh2ImageStudioFingerprint(companion);
...
async function vh2CreateTimeline(companion){
    ...
    for(const k of [...VH2_PROFILE_TRANSFER_FIELDS,'appearance','photoStyle','photoDirection','personality','behaviorExamples','description','backstory','chatStyle','textingStyle','conversationStyle','chatExamples','chatAvoid','chatLength','values','contradictions','vulnerabilities','relationshipStyle','habits','moodBaseline','startingRelationship','regulationProfile','conflictRecovery','emotionExpression','ruminationStyle','reactionTiming','emotionalGranularity'])if(companion[k]!==undefined)profile[k]=safeJsonClone(companion[k]);
    await vh2Enqueue(timeline,'create_profile',{name:companion.name.slice(0,80),profile,personaId,companionId:companion.id,providerScope:'horde:'+companion.id,deferAdvance:true,deferOpening:true});
    ...
    if(companion.openingMode==='vh_first'&&String(companion.openingMessage||'').trim())await vh2Enqueue(timeline,'configure_expression_profile',{fields:{openingMode:'vh_first',openingMessage:companion.openingMessage}});
...
async function vh2SyncStarterClips(companion,timeline=getActiveCompanionTimeline(companion.id)){
 ...
 const task=(async()=>{
  for(const clip of companion.startingVideoClips||[]){
   ...
   if(!await companionVideoJobSource(clip))throw Error('Starter clip media is missing. Upload it again in Edit human → Social presence → Starter clips.');
...
function vh2RenderSocial(companion,content,button,author=false){
    ...
    // Keep interactive DOM stable across life ticks and provider-status refreshes.
    // Replacing a pressed button loses its click; replacing a comment field loses editing state.
    const signature=JSON.stringify([timeline.id,author,companionSocialTab,companion.name,companion.profilePhoto,posts,photos,link.agencyPaused,link.socialSettings,link.postBefore,link.photoBefore,companion.startingSocialPosts?.length]);
    if(content._vhFeedSignature===signature&&content.querySelector('.vh-feed-profile, [data-social-post]'))return;
    if(content.contains(document.activeElement)&&document.activeElement.matches('.vh-feed-comments input')){
        document.activeElement.addEventListener('blur',()=>setTimeout(()=>{if(content.isConnected)renderCompanionSocialPanel(companion);},0),{once:true});
        return;
    }
    content._vhFeedSignature=signature;
...
function vh2LifestylePanels(host,companion,timeline){
 ...
 if(fp){const controls={};for(const [key,label] of [['enabled','Enable recurring finances'],['incomePerHour','Income per hour of scheduled work'],['dailyIncome','Recurring daily income'],['dailyExpense','Recurring daily expenses'],['debtRepaymentRate','Percent of new income used for unpaid expenses']])controls[key]=field(finance,label,fp[key]??25,typeof fp[key]==='boolean'?'checkbox':'number');controls.debtRepaymentRate.min='0';controls.debtRepaymentRate.max='100';const work=[];for(const p of link.travelPlaces||[])work.push([p.id,field(finance,'Paid work place: '+p.label,fp.workPlaceIds.includes(p.id),'checkbox')]);button(finance,'Save finances',b=>send('configure_finances',{policy:{...Object.fromEntries(Object.entries(controls).map(([k,i])=>[k,i.type==='checkbox'?i.checked:Number(i.value)])),workPlaceIds:work.filter(([id,i])=>i.checked).map(([id])=>id)}},b));note(finance,`Currency: ${link.gifts?.currency||'USD'}. Unpaid expenses: ${(link.finance.unpaid||0).toFixed(2)}. Starting funds are not reset by this form.`);for(const e of (link.finance.ledger||[]).slice(-8).reverse())note(finance,`${e.kind.replaceAll('_',' ')}: ${e.currency} ${(e.amountMinor/100).toFixed(2)} · ${e.detail}`);}
