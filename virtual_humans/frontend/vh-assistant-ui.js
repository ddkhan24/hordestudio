...
const VH_AI_HELPERS = [
 ...
 {id:'identity',title:'Person & background',sections:[],editorTab:'cs-identity',hint:'Open the Person page and generate only the fields you choose.'},
...
function vhAIHelpers(companion,selected='identity'){
 ...
 const update=()=>{const h=VH_AI_HELPERS.find(x=>x.id===f.elements.helper.value),opensEditor=h.id==='chat'||!!h.editorTab;f.querySelector('[data-help]').textContent=h.hint;for(const key of ['direction','model']){f.elements[key].closest('label').hidden=opensEditor;f.elements[key].disabled=opensEditor;}f.querySelector('.form-hint').hidden=opensEditor;f.querySelector('.form-hint').textContent=h.sections.length+' section request'+(h.sections.length===1?'':'s')+' using this character’s text provider. Review before applying. No photos are generated.';f.querySelector('[type=submit]').textContent=h.editorTab?'Open '+h.title.toLowerCase():h.id==='chat'?'Open writing style page':'Draft with AI';};f.elements.helper.onchange=update;update();
 f.onsubmit=e=>{e.preventDefault();const h=VH_AI_HELPERS.find(x=>x.id===f.elements.helper.value),direction=f.elements.direction.value.trim(),model=f.elements.model.value.trim();d.close();if(h.editorTab){switchView('companionStudio');openCompanionStudio(companion.id);activateCompanionStudioTab(h.editorTab);return;}if(h.id==='chat'){switchView('companionStudio');openCompanionStudio(companion.id);activateCompanionStudioTab('cs-chat-style');return vhOpenPageBuilder('cs-chat-style');}if(h.id==='wardrobe'&&!direction)return vhWardrobeBuilder(companion);void vhGenerateEssentials(companion,h.sections,{direction,model});};
}
