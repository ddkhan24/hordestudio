...
from virtual_humans.backend.vh2_feed_discovery import choices, discover, preview
...
class Discovery(unittest.TestCase):
 ...
 def test_feed_preview(self):
  with patch('virtual_humans.backend.vh2_feeds.fetch',return_value=b'<rss><channel><item><title>Local news</title></item></channel></rss>'):
   result=preview({'url':'https://example.org/rss'})
   self.assertTrue(result['valid']);self.assertEqual(result['titles'],['Local news'])
  with patch('virtual_humans.backend.vh2_feeds.fetch',return_value=b'<html>News homepage</html>'):
   with self.assertRaises(ValueError):preview({'url':'https://example.org'})
 def test_transport(self):
  with patch('virtual_humans.backend.vh2_feeds.fetch',return_value=b'[]') as fetch:
