'use strict';
const assert=require('node:assert/strict'),e=require('../virtual_humans/engine/vh2-people-engine');
...
c=fixture();a=c.vh2People.actors.sam;a.hunger=99;a.energy=90;a.policy.temperature=1;e.advance(c,start,local);assert.equal(a.action.kind,'eat');assert.equal(a.balance,30,'home food is covered by the actor daily-expense model');e.advance(c,start+20*MIN,local);assert.ok(a.hunger<50);assert.ok(a.balance>=28,'home meals do not double-charge daily living costs');
