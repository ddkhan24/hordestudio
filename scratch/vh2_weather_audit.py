...
class Weather(unittest.TestCase):
 ...
 def test_rain_showers_are_not_snow(self):
  ...
  with patch('virtual_humans.backend.vh2_weather.fetch',return_value=json.dumps({'current':current}).encode()):self.assertEqual(vh2_weather.read([-.1,51.5])['description'],'Rain')
